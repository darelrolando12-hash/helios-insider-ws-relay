 ...
 */
const POLL_HARD_TIMEOUT_MS = 40_000;
...
async function _poll(ticker: string) {
  ...
  try {
    ...
    console.log(`[chainAggregator] ${ticker}: fetch took ${(_elapsedMs / 1000).toFixed(1)}s (${contracts.length} contracts)`);
    ...
    const _elapsedMs = performance.now() - _t0;
    console.error(`[chainAggregator] ${ticker} poll failed after ${(_elapsedMs / 1000).toFixed(1)}s: ${formatError(e)}`);
  } finally {
    _releaseSlot();
  }
}
