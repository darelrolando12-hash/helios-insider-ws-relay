...
function _scoreTicker(ticker: string) {
  ...
  // Gate 2: not halted. null means one of two different things:
  //   (a) Nasdaq-listed ticker, no halt event fired today — genuinely
  //       "not halted", the normal quiet-day state.
  //   (b) NYSE/Arca-listed or index ticker — Massive only publishes
  //       halt/resume indicators for Nasdaq-listed securities, so this
  //       ticker can NEVER report true here even during a real halt.
  //       null is data-unavailability, not a confirmed "not halted".
  // Either way, scoring proceeds unless isHalted() === true — a real halt
  // is the only case that should ever block. See luldStore.hasHaltCoverage().
...
function _emit(signal: Signal) {
  ...
  for (const fn of _listeners) {
    try { fn(signal); }
    catch (e) { console.error(`[confluenceEngine] Listener error: ${formatError(e)}`); }
  }
}
