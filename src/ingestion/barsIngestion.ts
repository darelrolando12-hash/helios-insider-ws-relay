...
async function upsertBars(
  ...
  const rows = bars.map((b) => ({
    ...
    v:    b.v != null ? Math.round(b.v) : null,
    vw:   b.vw ?? null,
    n:    b.n  ?? null,
  }));
...
export async function runBarsDailyBackfill(client: MassiveRestClient): Promise<void> {
  ...
  for (const gap of KNOWN_GAPS) {
    try {
      ...
      console.error(`[barsIngestion] ${gap.ticker} targeted gap fill: unexpected error — ${formatError(e)}`);
    }
  }
  ...
  for (const ticker of BARS_DAILY_TICKERS) {
    try {
      ...
      console.error(`[barsIngestion] ${ticker}: unexpected error — ${formatError(e)}`);
    }
  }
