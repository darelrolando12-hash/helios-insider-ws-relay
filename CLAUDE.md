...
**The free-float fix (W8, `545eed7`) has never run in production: its table doesn't exist.** Railway logs on both the 2026-09-10 and 2026-09-11 deployments: `[shortInterestIngestion] free float hydrate failed — relation "public.stock_float" does not exist`. Shadow mode hides the write side (`WOULD UPSERT stock_float`), so the only visible symptom is that hydrate line. Whatever reads short interest as a percent of float is still on the pre-W8 path. Needs the table's migration applied in Supabase before browser writes are switched off.
