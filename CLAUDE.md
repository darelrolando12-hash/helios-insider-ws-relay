...
**Confirmed closed, 2026-09-13, with the full row list rather than a pass/fail check.** `signal_outcomes` now has exactly three policies — `anon_insert_outcomes`, `anon_select_outcomes`, and the one `signal_outcomes_update` restricted to rows whose parent signal is still `pending`; `anon_update_outcomes` (the old unconditioned one) is gone. Raw grants are exactly INSERT/SELECT/UPDATE. **Not yet confirmed:** the live-write-path check (`outcomeResolver`'s 60-second poll still succeeding under the new policy) was skipped because the market was closed at fix time — run it the next session `outcomeResolver` has a pending signal aged past 5 minutes, before assuming the policy doesn't also block the legitimate write.
