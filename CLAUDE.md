...
Fixed by adding the REVOKE to both DDL files; `observationTables.schema.test.ts` now fails if a table's GRANT block has no `REVOKE ALL … FROM anon` before it.

**Checked on `signals`/`signal_outcomes` too, and one was real.** `signals`' own UPDATE policy already narrows to one state transition (`status='pending' → status IN ('resolved','expired')`) — it was already safe, raw grant notwithstanding. `signal_outcomes` was not: `using (true)`, `with check (true)`, unconditioned — anyone with the anon key could rewrite `pnl_pct`, `result`, `exit_price` on any resolved signal's outcome, the ground truth Brain's win rates and every backtest-vs-forward comparison reads. Unlike `gex_regime_log`, column-scoping doesn't work here — `outcomeResolver.ts`'s upsert legitimately writes every column on every call — so the fix mirrors `signals`' own state-machine discipline instead: a write is only allowed while the outcome's *parent signal* (via `signal_id`) is still `status = 'pending'`, which is exactly outcomeResolver's own precondition (it only ever queries `WHERE status = 'pending'`, and a signal's status never returns to pending once resolved or expired). Fix: `backups/signal-outcomes-permissions-fix.sql`.

**Round 1 of that fix was itself incomplete — `DROP POLICY IF EXISTS <name>` only removes a policy under that exact name.** The corrective SQL added a new, restricted UPDATE policy but assumed any pre-existing one would share its name; it didn't — `signal_outcomes` already had an unrestricted one from whenever the table was first set up, named `anon_update_outcomes`, unknown until Wegic ran a full `pg_policies` listing and reported it back rather than trusting the "expected" result in the verify comment. Postgres runs multiple PERMISSIVE policies for one command as an OR — a row passes if it satisfies *any* of them — so the new restricted policy sat beside the old unconditioned one and changed nothing; the table was exactly as open as before. **Never assume a pre-existing policy's name on a table you didn't create.** List every policy on the table first (`select policyname, cmd from pg_policies where tablename = '<table>'`) and drop each one that competes with the new rule, by its real name.

**Confirmed closed, 2026-09-13, with the full row list rather than a pass/fail check.** `signal_outcomes` now has exactly three policies — `anon_insert_outcomes`, `anon_select_outcomes`, and the one `signal_outcomes_update` restricted to rows whose parent signal is still `pending`; `anon_update_outcomes` (the old unconditioned one) is gone. Raw grants are exactly INSERT/SELECT/UPDATE. **Write path confirmed 2026-09-15, on the migrated database** (see INFRASTRUCTURE): as `anon`, an update to an outcome whose parent signal is `resolved` affected 0 rows, and the same update to one whose parent is `pending` affected 1 — the value written back unchanged both times, so the test could not alter data. That exercises the policy with the role `outcomeResolver` uses; the first live session still has to show the resolver's own writes landing.
...
`src/{engines,stores,ledger,state,lib}` is the **browser's** copy, and since 2026-09-15 **it is live**: the production frontend is built from this repo (see INFRASTRUCTURE). Until then the live frontend was Wegic's and this directory was a reference snapshot — which is why older notes call it "frozen". Railway's Root Directory is `relay/` (boot logs show `> helios-insiders-relay@1.0.0 start`, `relay/package.json`'s name), so the two copies deploy to different places from the same `main`. They have drifted (17 of 22 overlapping files differed as of 2026-08-27).

**Edits to `src/` now reach production on merge to `main`, and `src/` is what writes signals** while `ENGINE_MODE=shadow`. The old rule was the opposite ("do not edit it believing it's live") — it is live now. The browser copy is still scheduled for deletion at Shadow Mode cutover, in the same step browser writes are disabled.

---

## INFRASTRUCTURE — off Wegic since 2026-09-15

- **Frontend: Vercel**, project `helios-insider-ws-relay`, root directory = repo root, `pnpm run build` from `main`, served at `helios-insiders.com` (Cloudflare DNS, records **DNS only** — proxying breaks Vercel's certificate). Deployment Protection covers `*.vercel.app`; the custom domain is public. Previews build from every pushed branch.
- **`main` deploys both Railway and Vercel.** Until Railway's Watch Paths is set to `relay/**`, a frontend-only commit to `main` restarts the trading engine.
- **The build type-checks nothing.** `npm run build` is `tsc && vite build`, and bare `tsc` exits 0 on the root tsconfig (`"files": []` + references). Run `npm run typecheck` yourself; enabling it in the build first needs the missing `vitest` types in `src/__tests__` resolved.
- **`VITE_*` is inlined into the public bundle at build time.** Never put a secret there, and changing one does nothing until a redeploy. `VITE_MASSIVE_API_KEY` was set on Vercel and would have published the Massive key; news now goes through the relay's `/rest/` proxy and the variable is deleted.
- **Database: Supabase project `jbzkzunohgivcifckwxi`**, Darel's own. The previous database was **Wegic-hosted** (`bqwlr9yi38q6pc.cloud.wegic.net`, Postgres ports filtered, so no `pg_dump`). Rebuilt over PostgREST: `backups/export-source-db.mjs` → `generate-schema.mjs` (types/PKs from the OpenAPI spec, unique keys from every `onConflict:`, indexes from the filters the code issues) → `generate-permissions.mjs` (RLS on all 21 tables, REVOKE before GRANT) → `load-into-new-db.mjs`. 32,557 rows, every table matched to its source count. Cut over and verified 2026-09-15: engine ingestion *read* a row count only the new database holds, and the engine *wrote* 989 `gex_regime_log` rows and 11 `engine_shadow_signals` rows there during the next session. Ingestion "upserted N" lines prove nothing about writes — in shadow mode those go through the intercepted client (see KNOWN GAPS). The browser's full write path (signal insert, outcome insert and conflict-update, pending → expired) was run as `anon` inside a rolled-back transaction and succeeded.
- **Market data is partially copied.** `bars_daily` complete, `daily_high_low` ~315k of 1,259,500, `bars_1m` not started (8.7M). Resume with `backups/copy-market-data.mjs` — keyset cursor checkpointed outside the database, `ON CONFLICT DO NOTHING`. Do **not** re-ingest instead: an empty `daily_high_low` makes `dailyHighLowIngestion` backfill 371 days across the allowlist at boot.
- **Wegic:** nothing points at it. Keep the account until a live session confirms the signal path — it is the only copy of the pre-migration database.

**`signals.pre_cvd_fix`** (added in the migration, mirrors `pre_cooldown_fix`) is `true` for every signal written before **2026-09-15T05:07:35Z** — the first moment a frontend carrying the session-scoped CVD fix served users. The fix commit (`0f0758d`, 2026-09-11) is the wrong boundary: that code never reached the Wegic frontend. All 4,509 migrated signals are marked. **Any win rate, Brain bucket or backtest-vs-forward comparison must filter on it**, and together with `signal_outcomes` being 99.0% `scratch` (thresholds too wide for 5–60 minute moves — P&L is computed, only 0.6% is exactly 0), there is currently no trustworthy outcome history to train on.
...
*Added 2026-09-15, measured during the first session after the database move.*

**The relay saturates during market hours, and it did so before the move.** Monday 09-14 (pre-cutover deployment) logged ~1,000–1,200 chain `poll failed` per session hour; 09-15 had 78–91% of chain polls failing from 16–18Z. From outside, `/health` took 4.7–25 s or timed out, while Massive answered the same snapshot endpoint directly in 0.4 s and Supabase in 0.2 s. Browser WebSockets closed with 1006 every few minutes and took ~38 s to reconnect; the engine logged `fetch failed` to both Massive and Supabase (33–138/hour on Monday too). That pattern points at the single relay-plus-engine process, not the providers. Closing a browser tab gave only intermittent recovery. Not root-caused — needs profiling outside a session, never a blind mid-session deploy. While it lasts, GEX/walls from chain polls are frequently stale.

**In shadow mode only the browser persists signals, live `bars_1m`, and outcome resolution.** Engine writes through `supabase` are intercepted (`[shadow] WOULD UPSERT bars_1m`, present on the pre-cutover deployment too), and the engine's `outcomeResolver` re-logs "expired" for the same pending signals every pass (3,000 lines on Monday) because its update never executes. With no browser open during a session, those tables simply don't advance: 09-15 produced 0 signals and SPY-only pre-market bars for that reason, not because of the migration (Monday's 390 bars per ticker came from browsers). The engine really writes only `gex_regime_log` and `engine_shadow_signals`, through the ungated observation client.

**The new Supabase instance is much smaller than Wegic's was.** `shared_buffers` 224MB, `max_connections` 60. `count(*)` on `bars_1m` (2.88M rows) took 10.2 s as `postgres`; the old host counted 8.7M in 0.57 s. `anon` has `statement_timeout=3s`, so any `count: 'exact'` over a large table returns HTTP 500 — `bars1mIngestion.getConfirmedCount` does exactly that. Indexed per-ticker queries are fine (232 ms). Check compute size before `bars_1m` reaches its full 8.7M rows or anything depends on a full scan.

**The `signals` UPDATE policy silently blocks `user_confirmed`.** It allows only pending → resolved/expired, so ScannerCockpit's `user_confirmed=true` on a pending signal affects 0 rows and only warns (reproduced as `anon` in a rolled-back transaction). It mirrors the source database's state-machine rule; a column-scoped allowance is needed if that flag matters.
...
**Fixed 2026-09-15 — `/engine/gex` and `/engine/delta` had no reader in production.** The endpoints were live on the relay from 2026-09-12, but their readers (`src/lib/serverGex.ts`, `src/lib/serverDelta.ts`) existed only in this repo while Wegic served the frontend. With the frontend now built from this repo, the production bundle at `helios-insiders.com` contains both endpoint paths (checked by fetching the deployed bundle).
