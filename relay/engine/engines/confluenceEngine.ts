 ...
 *
 * Real, measured defect (2026-09-10), found in the browser engine that is
 * live today and mirrored here so this engine is correct before it ever
 * leaves shadow mode: the edge-triggered gate compared bands with a bare
 * equality check, so a score oscillating a point either side of a threshold
 * re-triggered on every crossing. The last full session carried 426 EXIT
 * rows against 3 ENTERs; EXIT is the 55–64 band, and 55 is the most-hovered
 * boundary on the scale.
 *
 * A Schmitt trigger, not a threshold change: it cannot make a signal fire
 * that would not have fired before, it only stops the same crossing being
 * counted repeatedly. 3 points is ~5% of the 55-point floor — wider than
 * tick-to-tick scoring jitter, far narrower than any band.
 */
 ...
 * in. Pure and exported so the latch is directly testable.
 *
 *   rising (or staying): the plain threshold applies — behaviour unchanged.
...
function _scoreTicker(ticker: string) {
  ...
  if (!signalType) {
    // Second oscillation path. This used to reset the latch to 'none' on ANY
    // null signal type — but resolveSignalType depends on CVD, GEX context
    // and price as well as score, so jitter in any of those flipped it
    // null↔non-null while the score never left its band, and the next tick
    // re-emitted. Release the latch only once the SCORE has left the bands.
